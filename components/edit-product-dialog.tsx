      ...
      if (result.success) {
        ...
        if (onProductUpdated) {
          try {
            ...
            console.error(
              "Помилка при оновленні списку продуктів:",
              refreshError
            );
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Змініть інформацію про продукт та натисніть "Зберегти зміни"
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Сума винагороди за виготовлення одиниці продукції
            </p>
          </div>
          <div className="space-y-2">
            <Label htmlFor={`cost-${product.id}`}>Вартість</Label>
            <Input
              id={`cost-${product.id}`}
              name="cost"
              type="number"
              ...
              value={formData.cost}
              onChange={handleChange}
              placeholder="Введіть вартість продукту (необов'язково)"
            />
            <p className="text-xs text-muted-foreground">
